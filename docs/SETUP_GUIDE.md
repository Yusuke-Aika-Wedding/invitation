...
`setup` を実行すると、N列「ご祝儀」に `入金済み`・`再入金`・`未入金` のプルダウンが設定され、IDがある既存行の空欄は `未入金` になります。新しいゲスト行は `未入金` を選択してください。

- `未入金`：出席回答済みゲストは送金先情報を表示できます。
- `入金済み`・`再入金`：GAS側で送金先情報の取得を拒否し、招待状にも表示しません。
- ゲストが確認画面で「はい、送金しました」を押すと `入金済み` へ更新されます。

`入金済み`・`再入金` への変更時に送る確認メールは、現時点では未実装です。今後追加します。
...
8. 出席回答済み・N列が `未入金` のゲストで、送金先表示ボタンからプレースホルダー情報が表示されることを確認する。
9. 「送金しました」→「本当に送金しましたか？」→「はい、送金しました」の順に押し、N列が `入金済み` になることを確認する。
10. ページを再読込し、送金先情報が表示されないことを確認する。
11. N列を `再入金` に変更した場合も、送金先情報が表示されないことを確認する。
12. ハンバーガーメニューから「招待状ページ」「2人の紹介ページ」を開く。
13. GASの `トリガー` 画面で、`sendReminderEmails` と `sendAfterReceptionThanksEmails` の所有者が専用アカウントであることを確認する。
