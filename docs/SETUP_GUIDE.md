...
現在の送信元・BCCは、どちらも `yusuke.aika.wedding@gmail.com` です。GASの実行ユーザーが別のGoogleアカウントの場合は、実行ユーザーのGmailでこのアドレスを「他のメールアドレスとして送信」に追加し、確認を完了してください。未確認のアドレスから誤送信しないよう、送信エイリアスが見つからない場合はメール送信を停止します。
