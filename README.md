...
- 招待状ページ
- ゲスト一覧N列で設定できる、ゲストごとの「Dear Guest」メッセージ
- スマホ・タブレット・PC対応のレスポンシブデザイン
- 大きく滑らかなフェード表示、桜の花びら、5秒ごと・スワイプ操作の写真スライド
- 結婚式までのカウントダウン
...
- 専用シート「WEDDING PREDICTION」での投票結果管理（ID・ゲスト名・質問ID・投票・投票日時）
- 初期表示された氏名を編集して回答に反映
- アレルギー「あり／なし」必須選択と、「あり」の場合だけ表示される詳細欄
- GASによる回答保存、確認メール、リマインドメール、参加御礼メール

Dear Guestは出欠・開催日時・来場感謝サイト訪問記録で切り替わります。N列の個別文は「回答済み・出席・結婚式2日以上前」の場合だけ優先します。開始・終了日時は「公開設定」B3・B4でいつでも変更できます（Dear Guest専用、メール送信日時は変更しません）。
